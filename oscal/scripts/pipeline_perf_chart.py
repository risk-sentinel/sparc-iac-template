...
Data source: docs/dev/pipeline-perf-history.json — a persistent time-series
file that accumulates one entry per successful workflow run. On each CI
invocation this script appends the latest runs from the GitHub API
(deduplicated by run ID) so the history grows over time and is never lost
to API pagination limits.

Usage:
    python3 oscal/scripts/pipeline_perf_chart.py \
        --baseline docs/dev/pipeline-baseline.json \
        --output docs/dev/pipeline-performance.png \
...
HISTORY_PATH = "docs/dev/pipeline-perf-history.json"


def get_recent_runs(limit=200):
...
def generate_chart(baseline_data, all_runs, output_path):
...
def main():
    ...
    parser.add_argument("--output", default="docs/dev/pipeline-performance.png")
