...
- **The SBOM lane's verification no longer fails on a working emit.** It
  confirmed the upload by reading the object back, which the evidence-emit role
  cannot do — those roles hold write access to their own prefix and nothing
  else, so the check could never pass, and it reported the object as *missing*
  when it was merely unreadable. The upload now proves itself: the write returns
  the stored object's identifier, which exists only if the object was created.
  Evidence was landing correctly throughout; this corrects the job's verdict on
  it.

## [0.2.2] — 2026-09-22

Makes the evidence lanes work in a fork, and makes them tell you when they do
not. `v0.2.1` repaired the two lanes that could not run; running them revealed
that one of them was filing nothing while reporting success, and that two others
could not run outside the organisation that publishes this template at all.

### Fixed

- **The SBOM lane emits.** It read a role-ARN secret under a name that is a
  reusable workflow's *parameter*, not a secret — so it resolved to nothing. The
  failed role assumption was then hidden twice: `continue-on-error` reported the
  step as successful, and the upload it guarded was skipped rather than failed.
  The lane scanned, signed and verified on every run, and wrote no object.
- **Scans no longer need a private container image or Docker Hub credentials.**
  `validate.yml` and `template-scans.yml` ran inside an image built by a
  repository you cannot see, pulled with credentials you do not have — so two of
  the eight workflows here could never have run in your fork. They now run on the
  hosted runner with pinned tool versions.
- **Evidence emission is all-or-nothing, and says so.** If you enable emission,
  a missing bucket or an unassumable role now **fails the run** and names what is
  missing, instead of quietly skipping the upload. If you do not enable it,
  nothing runs and nothing complains. The half-configured middle — which produced
  green runs that filed nothing — is gone.
- **The SBOM lane confirms its objects landed** with `head-object` after the
  upload, rather than trusting the exit code.
- **A permanently inert job was removed.** `validate.yml` carried a sanitization
  check for a directory and a script that are both absent from this template; it
  could only ever exit early.
- `anthropics/claude-code-action` is pinned by digest rather than by tag.

### Notes for adopters

- Requirements are unchanged: `EVIDENCE_EMIT_ENABLED`, `COMPLIANCE_S3_BUCKET`
  and an assumable OIDC role. What changed is the failure mode — a partial
  configuration is now reported rather than silently tolerated.
- Tool versions in the scan workflows are pinned deliberately. A scan is a
  verdict, and an unpinned install makes that verdict depend on release timing.
