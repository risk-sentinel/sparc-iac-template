...
def evaluate_trufflehog(path, max_secrets):
    ...
    if not path or not os.path.exists(path):
        return {"count": 0, "passed": True, "details": "Not run"}
...
def evaluate_semgrep(path, max_high, max_medium):
    ...
    if not path or not os.path.exists(path):
        return {"high": 0, "medium": 0, "low": 0, "passed": True, "details": "Not run"}
...
def evaluate_pip_audit(path, max_critical, max_high):
    ...
    if not path or not os.path.exists(path):
        return {"critical": 0, "high": 0, "total": 0, "passed": True, "details": "Not run"}
