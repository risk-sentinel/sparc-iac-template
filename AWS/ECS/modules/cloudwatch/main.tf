...
resource "aws_cloudwatch_log_metric_filter" "rails_error" {
  ...
  log_group_name = var.ecs_log_group_name
  # Match genuine app errors, EXCLUDE ActionController::RoutingError — those are
  # internet vulnerability scanners probing bogus paths (/wp-json, /ReportServer,
  # /wp-config.php.bak, POST /) that SPARC correctly 404s. They are security
  # noise, not application errors, and a scanner burst (>20/5min) was flapping
  # the rails-error-rate alarm. This surfaced with SPARC v1.13.1's structured
  # JSON logging (#785): every error line is now `{"level":"ERROR",...}` so the
  # literal token "ERROR" is present in the scanner RoutingErrors, which the
  # prior plain-text logs (`ActionController::RoutingError`, no uppercase ERROR)
  # never matched. Real route breakage surfaces as 5xx, which still counts.
  pattern = "\"ERROR\" -\"RoutingError\""
...
resource "aws_cloudwatch_log_metric_filter" "app_secret_access_sso" {
  ...
  # Alarm on AssumedRole reads of app-secrets EXCEPT the expected automated
  # readers, else it false-positives on every CI plan/deploy and every ECS task
  # start (#578): ci-execute (the #316-chained deploy role — the OIDC entry role
  # sparc-iac-github-actions never reads secrets directly post-chain, kept only
  # defensively) and ecs-execution-role (injects valueFrom secrets at task start,
  # the dominant reader). Humans (IAMUser/Root) are caught by the non-sso filter.
  pattern = "{ ($.eventName = \"GetSecretValue\") && ($.requestParameters.secretId = \"*app-secrets*\") && ($.userIdentity.type = \"AssumedRole\") && ($.userIdentity.sessionContext.sessionIssuer.userName != \"sparc-iac-github-actions\") && ($.userIdentity.sessionContext.sessionIssuer.userName != \"example-ci-execute\") && ($.userIdentity.sessionContext.sessionIssuer.userName != \"example-ecs-execution-role\") }"
...
resource "aws_cloudwatch_metric_alarm" "app_secret_access" {
  ...
  alarm_description   = "App secrets (example/app-secrets) was accessed — verify authorized usage. Auto-resets ~60s after last access."
...
resource "aws_cloudwatch_log_metric_filter" "app_secret_modify_sso" {
  ...
  # Same intent as the access-sso filter, for secret MODIFICATIONS. Excludes
  # ci-execute (terraform manages the app-secrets version on deploy). The
  # ecs-execution-role only READS at task start (never modifies), so it is NOT
  # excluded here — a modify by it would be genuinely unexpected. (#578)
  pattern = "{ ($.eventName = ${local.secret_modify_events}) && ($.requestParameters.secretId = \"*app-secrets*\") && ($.userIdentity.type = \"AssumedRole\") && ($.userIdentity.sessionContext.sessionIssuer.userName != \"sparc-iac-github-actions\") && ($.userIdentity.sessionContext.sessionIssuer.userName != \"example-ci-execute\") }"
...
resource "aws_cloudwatch_metric_alarm" "app_secret_modify" {
  ...
  alarm_description   = "App secrets (example/app-secrets) was modified (PutSecretValue, UpdateSecret, DeleteSecret, etc.) — verify authorized change. Auto-resets ~60s after last modification."
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
