...
resource "aws_db_instance" "main" {
  ...
  # CKV2_AWS_60 (#518) — carry instance tags onto automated + manual snapshots so
  # a restored snapshot inherits the governance tags (Boundary/Environment/Repo)
  # rather than landing untagged. Free, applies in-place with no downtime.
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Principal = {
          Service = "example-db-0001.rds.amazonaws.com"
        }
      }
    ]
  })
...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "rotation" {
  ...
  # Pin the SAR app version so neither a CI plan nor an operator apply can
  # silently jump to "latest" (which would need a change set and could
  # re-provision the rotation lambda). Matches the currently deployed version.
  semantic_version = "1.1.647"
  ...
  parameters = {
    ...
    # The SAR template defaults runtime to python3.10, which AWS retired
    # (#531 Phase 0). Pin to a supported runtime. The live value was applied
    # out-of-band via a SAR stack update by an operator (the deploy role can't
    # change-set this stack — see ignore_changes); kept here as the codified
    # desired value so any future recreate lands on python3.12.
    runtime = "python3.12"
  }

  # This AWS-managed serverlessrepo construct is operator-managed out-of-band:
  # the CI deploy role (ci-execute) lacks serverlessrepo:CreateCloudFormationChangeSet,
  # so any parameter / version / tag change must be applied by an operator, not
  # deploy-on-merge. Ignore those attributes so CI never attempts (and fails) a
  # change set — nor reverts the operator's runtime bump. (#445 tags; #531 runtime/version.)
  lifecycle {
    ignore_changes = [tags, tags_all, parameters, semantic_version]
  }
}
...
resource "aws_iam_role_policy" "rotation_dlq_send" {
  ...
  # `role` derives from data.aws_lambda_function.rotation, which depends_on the
  # serverlessrepo CFN stack — so it resolves "known after apply" each plan and,
  # because `role` is ForceNew, perpetually forces a REPLACE (which also needs
  # iam:DeleteRolePolicy on the serverlessrepo role). The role NAME is stable
  # (from the fixed function name), so ignore the drift to stop the spurious
  # replace. Surfaced by the #445 full-apply.
  lifecycle {
    ignore_changes = [role]
  }
}
