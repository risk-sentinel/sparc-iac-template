...
pass() { local msg="$1"; echo -e "  ${GREEN}PASS${NC} $msg"; PASS=$((PASS + 1)); }
fail() { local msg="$1"; echo -e "  ${RED}FAIL${NC} $msg"; FAIL=$((FAIL + 1)); }
warn() { local msg="$1"; echo -e "  ${YELLOW}WARN${NC} $msg"; WARN=$((WARN + 1)); }
header() { local msg="$1"; echo -e "\n${YELLOW}=== $msg ===${NC}"; }
...
validate_ssp_assembly() {
    ...
    for pattern in ecs ec2 azure-vm; do
        case $pattern in
            ...
            *) ;; # unreachable — for loop only yields the 3 patterns above (S131)
        esac
...
run_checkov() {
    ...
    result=$(checkov -d "$dir" --framework terraform \
        --output json --output sarif --output junitxml \
    ...
    if [[ -f checkov-results/results_json.json ]]; then
        ...
        # JUnit XML for the junit2hdf converter work in the profile repos. The
        # extension names the FORMAT, as .sarif does — a bare .xml would not say
        # which XML this is. Guarded because the script runs under `set -e`: if a
        # checkov release ever stops emitting this file, a bare mv would abort
        # the whole validation run over a supplementary output.
        if [[ -f checkov-results/results_junitxml.xml ]]; then
            mv checkov-results/results_junitxml.xml \
                "checkov-results/${type}_${epoch}_results.junit.xml"
        else
            warn "Checkov $type produced no JUnit XML (json/sarif unaffected)"
        fi
