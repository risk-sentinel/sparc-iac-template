...
# ---------------------------------------------------------------------------
# Azure Bastion Subnet (#9) — opt-in. Azure requires the exact name
# "AzureBastionSubnet" and a minimum /26. Created only when enable_bastion.
# ---------------------------------------------------------------------------
...
# AzureBastionSubnet NSG — the rule set below is MANDATED by Azure Bastion; the
# service fails to deploy without exactly these allows. Internet:443 inbound and
# Internet:80 outbound are required by Bastion (control-plane + session info),
# not an open ingress to workloads. Default deny-all still applies to everything
# else. (checkov CKV_AZURE_* on the required Internet rules are accepted in
# checkov-baseline.yml with this rationale.)
