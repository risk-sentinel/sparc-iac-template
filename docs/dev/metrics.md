...
The FedRAMP 20x compliance pipeline emits a structured per-run metrics record covering every scanner, stored over time in `docs/dev/pipeline-perf-history.json` and rendered as trend panels in `docs/dev/pipeline-performance.png`.
...
post-scan          →  downloads both bundles
  (compliance.yml)  →  fetches  AWS Config  describe-compliance-by-config-rule
                    ...
                    →  writes  run-metrics.json   (per-run, S3-archived)
                             + pipeline-perf-history.json  (persistent, committed)
                    →  runs    oscal/scripts/pipeline_perf_chart.py
                    →  writes  pipeline-performance.png    (5 panels, committed)
```
...
| **Trivy** | Not included | Not included | Runs in `build-runner.yml` on a separate (monthly) schedule against the CI runner image, not per compliance run. Tracked separately via the Trivy scan summary in the build-runner step summary and `container-baseline.yml`. See gap note below. |
...
`docs/dev/pipeline-performance.png` has 5 panels top-to-bottom:
