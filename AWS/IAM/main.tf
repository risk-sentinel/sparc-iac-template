...
data "aws_iam_openid_connect_provider" "github" {
  count = (var.enable_scanner_role || var.enable_db_scanner_role || var.enable_db_scanner_runner || var.enable_cis_rhel9_golden_ami || var.enable_container_build_sign_publisher || var.enable_container_build_sign_sca_emit || var.enable_sparc_validate_sca_emit || var.enable_container_build_sign_sca_aggregate || var.enable_sparc_app_ci || var.enable_sparc_iac_sca_emit || var.enable_ci_chain || var.enable_profile_emit || var.enable_evidence_reader) ? 1 : 0
...
# Pull and inspect statements are both resource-scoped to an explicit list of
# six repositories (local.sparc_validate_ecr_repositories) so even if the
# scanner role is misused, it can only read SPARC's images, not other tenants'
# or unrelated workload images on the same account.
# Auth-token statement is unscoped per AWS API contract — the token is
# account-level, not resource-level.
# ---------------------------------------------------------------------------

locals {
  # Single source of truth for the repositories the sparc-validate scanner
  # assesses: the managed workload images (SPARC app + nginx sidecar), the
  # CI/auditor images, and the adopted heimdall2/vulcan repos. (custom-nginx
  # retired #515 — stale unsigned out-of-band repo, never deployed.)
  #
  # Both the pull and the inspect/signing statements below consume this list.
  # They used to carry independent resource scopes, which is how the inspect
  # statement silently ended up covering one repository of six (#633).
  sparc_validate_ecr_repositories = [
    local.name_prefix,
    "${local.name_prefix}-nginx",
    "sparc-auditor",
    "sparc-ci-runner",
    "heimdall2",
    "vulcan",
  ...
  sparc_validate_ecr_repository_arns = [
    for repo in local.sparc_validate_ecr_repositories :
...
data "aws_iam_policy_document" "sparc_validate_ecr_pull" {
  ...
  statement {
    ...
    # Read-only pull for the sparc-validate cis-docker leg (#514 / sparc-validate#238).
    resources = local.sparc_validate_ecr_repository_arns
  }

  # Inspect/describe reads incl. image signing status + OCI referrers (attestations/
  # SBOMs). simulate-principal-policy returned implicitDeny for these — the managed
  # SecurityAudit/ViewOnlyAccess don't cover the newer signing/referrer actions.
  # (ListImageReferrers is ecr:List*, not Describe*/Get*.)
  #
  # Scoped to the same six repositories as the pull statement above (#633). This
  # was previously `${local.name_prefix}-*`, which matches only `example-nginx`
  # — an IAM wildcard matches after the literal prefix, so `example-*` does not
  # match `example` itself, nor the adopted heimdall2/vulcan/auditor/runner
  # repos. The denial was invisible: SV-233065 reported genuinely signed images as
  # unsigned rather than erroring.
  statement {
    sid = "ECRInspectAndSigning"
    actions = [
      "ecr:Describe*",
      "ecr:Get*",
...
# ---------------------------------------------------------------------------
# sparc-validate Scanner — Compliance Artifacts Read (#278)
#
# Read-only access to attestation evidence in the security-artifacts bucket
# (provisioned by modules/logging as aws_s3_bucket.artifacts). Two prefixes:
#   attestations/      — CMSgov-pattern JSON for `alternative`-status controls,
#                        consumed by `saf attest apply` + the document_attestation
#                        resource (sparc-validate#115).
#   leveraged-systems/ — AWS Artifact evidence manifests for `inherited` controls.
#
# Intentionally narrow: GetObject/GetObjectVersion scoped to the two prefixes,
# ListBucket conditioned on the same prefixes, no Put/Delete. KMS decrypt is
# granted at the key-policy layer via kms:ViaService on the bucket's aws/s3
# managed key, so no IAM kms:Decrypt is required here (CMK migration → #145).
# ---------------------------------------------------------------------------

data "aws_iam_policy_document" "sparc_validate_compliance_artifacts" {
  count = var.enable_scanner_role && var.enable_artifacts_read_for_sparc_validate ? 1 : 0

  statement {
    sid     = "ReadComplianceArtifacts"
    actions = ["s3:GetObject", "s3:GetObjectVersion"]
    resources = [
      "arn:aws:s3:::${var.artifacts_bucket_name}/attestations/*",
      "arn:aws:s3:::${var.artifacts_bucket_name}/leveraged-systems/*",
    ]
  }

  statement {
    sid       = "ListComplianceArtifacts"
    ...
    condition {
      test     = "StringLike"
      variable = "s3:prefix"
      values   = ["attestations/*", "leveraged-systems/*"]
    }
  }

  # CMK-switch safety (#333). The bucket currently uses the AWS-managed
  # aws/s3 key, whose key policy already grants Decrypt to in-account
  # principals via-service — so reads work today without this statement.
  # But if the bucket is later re-encrypted under a customer-managed CMK
  # (a normal hardening step for a compliance evidence store, tracked in
  # #145), that blanket grant no longer applies and GetObject would 403
  # with KMS AccessDenied — silently flipping sparc-validate#115's
  # document_attestation controls Pass -> Fail. This grants kms:Decrypt
  # ONLY for S3-mediated calls (kms:ViaService), so it cannot decrypt
  # arbitrary keys directly; Resource stays "*" until a specific CMK ARN
  # exists, at which point #145 also adds the role to that CMK's key policy.
  statement {
...
resource "aws_iam_role_policy" "sparc_validate_compliance_artifacts" {
  count  = var.enable_scanner_role && var.enable_artifacts_read_for_sparc_validate ? 1 : 0
  name   = "compliance-artifacts-read"
  role   = aws_iam_role.sparc_validate[0].id
  policy = data.aws_iam_policy_document.sparc_validate_compliance_artifacts[0].json
}
...
data "aws_iam_policy_document" "sparc_validate_db_scanner" {
  ...
  # EC2 DescribeInstances/DescribeTags for scan provenance. Consumed cross-repo
  # by sparc-validate OIDC workflows. Describe* has no resource-level scoping,
  # so Resource="*" (#377). (sparc-validate-scanner already inherits these via
  # its SecurityAudit + ViewOnlyAccess managed policies; only this role needs it.)
  statement {
