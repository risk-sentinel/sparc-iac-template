...
# ---------------------------------------------------------------------------
# Hibernate-watchdog GitHub App credentials (#573)
#
# JSON {app_id, installation_id, private_key} for the sparc-iac-diagram-bot
# GitHub App. The watchdog Lambda reads this, signs a short-lived RS256 JWT,
# and exchanges it for an installation token to workflow_dispatch the
# hibernate/wake workflow. Gated so the shell only exists when the watchdog is.
#
# No aws_secretsmanager_secret_version here — the private key lives OUTSIDE
# Terraform state. Populate out-of-band per docs/dev/hibernate_watchdog.md.
# ---------------------------------------------------------------------------

resource "aws_secretsmanager_secret" "hibernate_watchdog_gh_app" {
  # No auto-rotation: holds an external GitHub App private key rotated at GitHub,
  # not via a Secrets Manager rotation Lambda (same posture as rotation-lambda-
  # token). Accepted finding tracked in checkov-baseline.yml (CKV2_AWS_57).
  count       = var.enable_hibernate_watchdog ? 1 : 0
  name        = "${local.name_prefix}/hibernate-watchdog-gh-app"
  description = "GitHub App credentials {app_id, installation_id, private_key} the hibernate watchdog uses to dispatch the hibernate/wake workflow. Populated out-of-band per docs/dev/hibernate_watchdog.md (#573)."
  kms_key_id  = var.kms_key_arn
