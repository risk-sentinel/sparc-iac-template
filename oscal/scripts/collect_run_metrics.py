...
output. Trivy container scans run in risk-sentinel/container-build-sign (the CI
runner image's owner) on a separate cadence and are not included here. See
docs/dev/metrics.md.
...
The history is NOT tracked in this repo (#615). Its canonical home is
``s3://your-security-artifacts-bucket/sparc/latest/sparc-iac/pipeline-perf-history.json``; CI
fetches it to ``perf-history.json`` before this runs and uploads it back
...
def _load_json(path: str) -> Any:
    ...
    try:
        with open(path) as f:
            return json.load(f)
    except (OSError, ValueError):  # JSONDecodeError is a ValueError subclass (S5713)
        return None
